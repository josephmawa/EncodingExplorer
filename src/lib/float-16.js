...
const ArrayPrototypeSymbolIterator = uncurryThis(
...
const ArrayBufferPrototypeGetByteLength = uncurryThisGetter(
  ArrayBufferPrototype,
  "byteLength"
);
const NativeSharedArrayBuffer =
...
const NativeTypedArrayPrototypeSymbolIterator =
...
function safeIfNeeded(array) {
  ...
  WeakMapPrototypeSet(
    arrayIterators,
    safe,
    ArrayPrototypeSymbolIterator(array)
  );
  return safe;
}
...
function convertToNumber(float16bits) {
  ...
  uint32View[0] =
    mantissaTable[offsetTable[i] + (float16bits & 0x3ff)] + exponentTable[i];
  return floatView[0];
}
  ...
  get(target, key, receiver) {
    ...
    if (
      WeakSetPrototypeHas(
        ...
        ObjectPrototype__lookupGetter__(target, key)
      )
    ) {
